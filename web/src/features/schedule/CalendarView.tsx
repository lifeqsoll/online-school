...
export function CalendarView({ events, mode = 'week', onModeChange }: Props) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12, gap: 12, flexWrap: 'wrap' }}>
        <Typography.Text>
          <a onClick={() => setAnchor((a) => a.subtract(1, mode === 'week' ? 'week' : 'month'))}>←</a>
          {' '}
          {mode === 'week'
            ? `${days[0].format('D MMM')} — ${days[6].format('D MMM YYYY')}`
            : anchor.format('MMMM YYYY')}
          {' '}
          <a onClick={() => setAnchor((a) => a.add(1, mode === 'week' ? 'week' : 'month'))}>→</a>
        </Typography.Text>
        {onModeChange ? (
          <Segmented
            value={mode}
        ...
        {selected ? (
          <div>
            <Typography.Paragraph>
              ...
              {selected.endsAt ? ` — ${dayjs(selected.endsAt).format('HH:mm')}` : ''}
            </Typography.Paragraph>
            {selected.course ? (
              <Typography.Paragraph type="secondary">{selected.course.title}</Typography.Paragraph>
            ) : null}
            {selected.meetingUrl ? (
              <Typography.Paragraph>
